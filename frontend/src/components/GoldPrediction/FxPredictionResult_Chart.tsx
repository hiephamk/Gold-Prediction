...
import { Box, Spinner, Text, VStack, HStack, Badge, Grid } from '@chakra-ui/react';
...
const FxPredictionResult_Chart: React.FC<FxActualPredictedDataProps> = ({ interval }) => {
  ...
  const fetch_predicted_prices = async () => {
    ...
    try {
      ...
      const normalized: OHLCPrediction[] = (data ?? [])
        .filter((p: any) => {
          // Filter out null/undefined entries
          if (!p || !p.date || p.open == null || p.high == null || p.low == null || p.close == null) {
            return false;
          }
          return true;
        })
        ...
        .filter((p: OHLCPrediction) => {
          const validNumbers = !isNaN(p.open) && !isNaN(p.high) && !isNaN(p.low) && !isNaN(p.close);
          const validRange = p.high >= p.low && p.open >= p.low && p.open <= p.high && p.close >= p.low && p.close <= p.high;
          const positiveValues = p.open > 0 && p.high > 0 && p.low > 0 && p.close > 0;
          return validNumbers && validRange && positiveValues;
        });
      
      const filtered = originalCount - normalized.length;
  ...
  useEffect(() => {
    ...
    const predictedCandleData = predictions
            ...
            <div style="padding: 12px; background: rgba(0,0,0,0.9); color: white; border-radius: 8px; min-width: 220px;">
              ...
              <div style="display: grid; gap: 4px; font-size: 12px;">
                ...
                <div style="display: flex; justify-content: space-between;">
                  ...
                  <span style="font-weight: 500;">$${range.toFixed(2)}</span>
                </div>
        ...
        {filteredCount > 0 && !loading && (
          <Box p={3} bg="orange.50" borderRadius="md" border="1px solid" borderColor="orange.200">
            <Text fontSize="sm" color="orange.800" fontWeight="medium">
              ℹ️ {filteredCount} invalid data point{filteredCount > 1 ? 's were' : ' was'} filtered out
            </Text>
          </Box>
        )}
...
//       console.log("raw predicted data:", data)

//       const normalized: OHLCPrediction[] = (data ?? [])
//       .filter((p: any) => p && p.date && p.open != null && p.high != null && p.low != null && p.close != null)
//       .map((p: any) => ({
//         date: toISO(p.date),
//         open: Number(p.open),
//         high: Number(p.high),
